...
int main(int argc, char* argv[]) {
  ...
  Env.PrepArgs(TStr::Fmt("\nMLSS'17: Simulate continuous time independent cascade model. build: %s, %s. Time: %s", __TIME__, __DATE__, TExeTm::GetCurTm()));
  TExeTm ExeTm;
  Try
  ...
  double AvCascSz = 0.0, StdError = 0; // average cascade size / standard error
  TIntH CascSzDistr, NumNodeInf, NumNodeInfDistr; // cascade statistics
  TFltPrV CascDurDistr; // cascade duration
  ...
  for (int c=0; c<NCascades; c++) {
	  ...
	  // generate cascade
	  TCascade C;
	  MLSS.GenCascadeCTIC(C, Verbose>0);
	  ...
	  CascDurDistr.Add(TFltPr(C.NIdHitH[C.NIdHitH.Len()-1].Tm, 1.0));

	  // if TMode==2, generate new random source
	  if (TMode==2) { MLSS.SetSources(NumSources); }
  }
  ...
  TGnuPlot::PlotValCntH(NumNodeInfDistr, TStr::Fmt("%s-nodeinf", OutFNm.CStr()), "", "Number of infections", "N", gpsLog10XY, true, gpwPoints, false, false);

  // plot cascade duration
  TGnuPlot::PlotValV(TGUtil::GetCCdf(CascDurDistr), TStr::Fmt("%s-cascdur", OutFNm.CStr()), "", "Cascade duration", "N", gpsAuto, false, gpwPoints);
